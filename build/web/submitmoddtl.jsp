<%-- 
    Document   : submitmoddtl
    Created on : Jun 11, 2015, 11:53:55 AM
    Author     : abc
--%>
...
<html>
    ...
    <%!int mod_ct=0,mod_code[];%><%!String name[],qual[],inst_code[],stream[],dobd[],dobm[],doby[],fname[],addr1[],addr2[],addr3[],addr4[];%>
    ...
    <%
           if(session.getAttribute("code1").equals(null)){%>
            ...
            mod_ct=Integer.parseInt(request.getParameter("mod_ct"));
            mod_code=new int[mod_ct];
            name=new String[mod_ct];
            for(int k=0;k<mod_ct;k++){
               mod_code[k]=Integer.parseInt(request.getParameter("mod_code"+k));
            ...
            for(int i=0;i<mod_ct;i++){
               //out.print("ccccccccccccccccccccccc"+student_ct+""+student_code[i]+""+name[i]+""+qual[i]+""+stream[i]+""+fname[i]+""+inst_cd[i]+""+addr1[i]+""+addr2[i]+""+addr3[i]+""+addr4[i]+""+addr_pin[i]+""+email[i]+""+phno[i]+""+pic_file[i]+""+username);
                ps = conn.prepareStatement("INSERT INTO OES_MODULE_MST(MODULE_CD,MODULE_DESC,CRT_BY) VALUES("+mod_code[i]+",'"+name[i]+"','"+username+"')");
                ps.executeUpdate();
               }
            } 
            catch (Exception e) {
    ...
    <body id="bcg" style="background-image: url(back6.jpg);opacity: 5">
        ...
        <form id="formmd" name="formmd" method="post" action="moduledtls.jsp">
            <input type="hidden" name="username" value="<%=username%>"/>
            <input type="hidden" name="emp_name" value="<%=emp_name%>"/>
            <input type="hidden" name="emp_dept" value="<%=emp_dept%>"/>
            <input type="hidden" name="emp_desg" value="<%=emp_desg%>"/>
