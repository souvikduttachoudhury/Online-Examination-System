<%-- 
    Document   : submitstuedit
    Created on : Jun 13, 2015, 10:46:04 AM
    Author     : abc
--%>
...
<html>
    ...
    <%
           ...
           // for(int k=0;k<student_ct;k++){
                ...
                dobd=request.getParameter("dobd");
                dobm=request.getParameter("dobm");
                doby=request.getParameter("doby");
           ...
           // for(int i=0;i<student_ct;i++){
               ...
               ps = conn.prepareStatement("UPDATE OES_STUDENT_MST SET STUDENT_NAME='"+name+"',STUDENT_QUALIFICATION='"+qual+"',INSTITUTE_CD='"+inst_cd+"',STUDENT_STREAM='"+stream+"',FATHER_NAME='"+fname+"',STUDENT_ADDR1='"+addr1+"',STUDENT_ADDR2='"+addr2+"',STUDENT_ADDR3='"+addr3+"',STUDENT_ADDR4='"+addr4+"',STUDENT_ADDR_PIN='"+addr_pin+"',STUDENT_EMAIL='"+email+"',STUDENT_PHNO='"+phno+"' WHERE STUDENT_CD="+student_code+"");
               ps.executeUpdate();
               //date="1111-01-01";
               if(!dobd.equals(""))
               {
    ...
    <body id="bcg" style="background-image: url(back6.jpg);opacity: 5">
        ...
        <form id="formsd" name="formsd" method="post" action="studentdtlselect.jsp">
            <input type="hidden" name="username" value="<%=username%>"/>
            <input type="hidden" name="emp_name" value="<%=emp_name%>"/>
            <input type="hidden" name="emp_dept" value="<%=emp_dept%>"/>
            <input type="hidden" name="emp_desg" value="<%=emp_desg%>"/>
