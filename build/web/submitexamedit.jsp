<%-- 
    Document   : submitexamedit
    Created on : Jun 29, 2015, 9:45:50 AM
    Author     : abc
--%>
...
<html>
    ...
    <%
           if(session.getAttribute("code1").equals(null)){%>
            ...
            conn.setAutoCommit(false);
               ...
               ps = conn.prepareStatement("UPDATE OES_EXAM_MST SET EXAM_CD_PREFIX='"+exam_cd_prfx+"',MODULE_CD="+mod_code+",CATEGORY_CD="+cat_code+",EXAM_PERIOD_MIN="+exam_min+",EXAM_VENUE='"+exam_venue+"',EXAM_TOT_MRK="+tot_mrk+",EXAM_TOT_STUDENTS="+tot_stud+",EXAM_NAME='"+exam+"',EXAM_PAPER_SETTER='"+paper_setter+"',EXAM_INSTR='"+instruction+"',UPD_BY='"+username+"' WHERE EXAM_CD="+exam_cd+"");
               ps.executeUpdate();
               date="1111-01-01";
               if(!exam_dtd.equals(""))
               {
               ...
               ps = conn.prepareStatement("UPDATE OES_EXAM_MST SET EXAM_DATE=TO_DATE('"+date+"','MM-DD-YYYY') WHERE EXAM_CD_PREFIX=? AND EXAM_CD="+exam_cd+"");
    ...
    <body id="bcg" style="background-image: url(back6.jpg);opacity: 5">
        ...
        <form id="formid" name="formexd" method="post" action="examdtlselect.jsp">
            <input type="hidden" name="username" value="<%=username%>"/>
            <input type="hidden" name="emp_name" value="<%=emp_name%>"/>
            <input type="hidden" name="emp_dept" value="<%=emp_dept%>"/>
            <input type="hidden" name="emp_desg" value="<%=emp_desg%>"/>
